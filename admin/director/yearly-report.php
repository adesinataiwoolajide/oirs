        ...
        <div class="col-md-12">
            ...
            <div class="tile">
                <div class="tile-body"><?php
                    $year = date("Y");
                    $min = $db->prepare("SELECT * FROM state_revenue WHERE year=:year  AND local_govt_id=:local_govt_id ORDER BY revenue_id desc");
                    $arrMin = array(':year'=>$year, ':local_govt_id'=>$local_govt_id);
                    $min->execute($arrMin);
                    if($min->rowCount()==0){ ?>
                        <h5><p style="color: red"  align="center">The Revenue Report For <?php echo $local_govt_name . " For ". date("Y") ?> is Empty</p></h5><?php
                    }else{ ?>
                        <div align="center">
                            <h1 style="color: green"><i class="fa fa-th-list"></i> State of Osun Revenue  Report</h1>
                            <p style="color: blue">OSUN STATE REVENUE REPORT FOR <?php echo $local_govt_name . " For ". date("Y"); ?></p>
                        </div>
