	...
	try{
		...
		if(isset($_POST['add_revenue'])){
			...
			if($bank->rowCount() == 0){
				...
				$new_balance = $befo + $revenue_amount;
				$vault_id = $see_bal['vault_id'];
				$hope = $localRevenue->updateVault($bank_name, $new_balance, $source_id, $vault_id);
			}
			if(!empty($localRevenue->addingStaffRevenue($local_govt_id, $staff_number, $source_id, $payer_name, $payer_phone, $payer_address, $receipt_number, $day, $month, $year, $bank_name, $bank_branch))AND (!empty($staffBiodata->insertingTheLastNumber($category, $numbers, $year_employ, $month)))){
