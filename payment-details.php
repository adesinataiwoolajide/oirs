<?php
    require 'header.php';
    require 'admin/ict-department/libs_dev/staff/staff_class.php';
    $staffBiodata = new staffBiodataIGR($db);
    require 'side-bar.php';
    require 'admin/ict-department/libs_dev/local-ministry/local_ministry_class.php';
    require 'admin/ict-department/libs_dev/revenue/revenue_class.php';
    $minLocal = new stateMinistry($db);
    $localMin = new localMinistry($db);
    $revSouceing = new sourcesOfRevenue($db);

    $reciept_number = $_GET['reciept_number'];
?>
<main class="app-content">
    <div class="app-title">
        <div>
            <h1 style="color: green"><i class="fa fa-th-list"></i> State of Osun Revenue Details</h1>
            <p style="color: blue"></p>
        </div>
        <ul class="app-breadcrumb breadcrumb side">
            <li class="breadcrumb-item active"><a href="payment-details.php?reciept_number=<?php echo $reciept_number ?>"><i class="fa fa-list"></i> Payment Details  </a></li>
            <li class="breadcrumb-item active"><a href="my-payment.php"><i class="fa fa-plus"></i> Check Payment</a></li>
            <li class="breadcrumb-item"><a href="./"><i class="fa fa-dashboard"></i> Home Page</a></li>
        </ul>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="col-md-12">
                <?php
                if((isset($_SESSION['success'])) OR ((isset($_SESSION['error'])) === true)){ ?>
                    <div class="alert alert-info" align="center">
                        <button class="close" data-dismiss="alert">
                            <i class="ace-icon fa fa-times"></i>
                        </button>
                     <?php include("../includes/feed-back.php"); ?>
                    </div><?php 
                }  ?>
            </div>
            <div class="tile">
                <div class="tile-body"><?php
                    $min = $db->prepare("SELECT * FROM state_revenue where reciept_number=:reciept_number");
                    $arr = array(':reciept_number'=>$reciept_number);
                    $min->execute($arr);?>
                    
                    <div align="center">
                        <h1 style="color: green"><i class="fa fa-th-list"></i> State of Osun Revenue Source List</h1>
                        <p style="color: blue"> CITIZEN REVENUE DETAILS</p>
                    </div>
                    <table class="table table-hover table-bordered" id="sampleTable">
                        <thead>
                            <tr>
                                <th>S/N</th>
                                <th>Source Name</th>
                                <th>Bank Name</th>
                                <th>Amount</th>
                                <th>Receipt Number</th>
                                <th>Payer Name</th>
                                <th>Time Paid</th>
                            </tr>
                        </thead>
                        <tfoot>
                            <tr>
                                <th>S/N</th>
                                <th>Source Name</th>
                                <th>Bank Name</th>
                                <th>Amount</th>
                                <th>Receipt Number</th>
                                <th>Payer Name</th>
                                <th>Time Paid</th>
                            </tr>
                        </tfoot>
                        <tbody><?php
                            $y=1;
                            while($see_ministry = $min->fetch()){
                                $staff_number = $see_ministry['staff_number'];
                                $source_id = $see_ministry['source_id'];
                                $local_govt_id = $see_ministry['local_govt_id'];
                                $localDetails = $localMin->gettingLocalGocyIDDetails($local_govt_id);
                                $sourceDetails = $revSouceing->seeRevenueourceID($source_id);
                                $ministry_id = $sourceDetails['ministry_id'];
                                $minDetails = $minLocal->gettingMinistryIDDetails($ministry_id);
                                $local_govt_code = $localDetails['local_govt_code']; ?>
                                <tr>
                                    <td><?php echo $y ?></td>
                                    <td><?php echo $revenue_source_name= $sourceDetails['source_name']; ?></td>   
                                    <td><?php echo $see_ministry['bank_name']. " ". $see_ministry['bank_branch'] ?></td>
                                    <td><?php echo "#". number_format($sourceDetails['revenue_amount']); ?></td>
                                    <td><?php echo $see_ministry['reciept_number']; ?></td>
                                    <td><?php echo $see_ministry['payer_name']; ?></td>
                                    <td><?php echo $see_ministry['time_added']; ?></td>
                                    
                                </tr><?php
                                $y++;
                            } ?>
                        </tbody>
                    </table>
                    <div class="row d-print-none mt-2">
                        <div class="col-12 text-right">
                            <a class="btn btn-primary" href="javascript:window.print();" target="_blank"><i class="fa fa-print"></i> Print Revenue List</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>
  
<?php require '../table-footer.php'; require '../alert.php'; ?>